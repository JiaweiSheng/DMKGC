"""Data container for a single-domain knowledge graph."""
...
class KnowledgeGraph(nn.Module):
    """Store triple splits and maintain local-to-global entity ID mapping."""
    ...
    def get_global_h_t(self, seeds, pre_langs, all_entity2kgs, all_entity_global_index):
        """Build global entity indices from alignment seeds and remap train/val/test IDs."""
    ...
    def _get_entity_global_index(self, seeds, pre_langs, all_entity2kgs, all_entity_global_index):
        """Assign global entity IDs: reuse IDs for aligned entities, create new IDs otherwise."""
    ...
    def _get_global_index(self, entity_ids, all_entity_global_index):
        entity_id_global = [all_entity_global_index[self.lang][int(e)] for e in entity_ids]
        return torch.LongTensor(entity_id_global)

    def generate_batch_data(self, h_all, r_all, t_all, batch_size, shuffle=True):
        triple_all = torch.cat([
            torch.unsqueeze(h_all, dim=1),
            torch.unsqueeze(r_all, dim=1),
            torch.unsqueeze(t_all, dim=1),
        ], dim=-1)
        return DataLoader(triple_all, batch_size=batch_size, shuffle=shuffle)
